...
export default function VirusPopup({ startDelay = 4000, videoElement }: VirusPopupProps) {
  ...
  const generatePopup = () => {
    ...
    const borderColors = ['border-[#FF00FF]', 'border-[#00FFFF]', 'border-[#00FF00]'];
    const color = isImage 
        ...
        {popups.map((popup) => (
          ...
          >
            {popup.isImage ? (
              ...
              <div className={`font-mono text-xl md:text-3xl p-4 bg-black border-4 border-[#00FF00] ${popup.color}`}>
                {popup.content}
              </div>
            )}
          </motion.div>
